...
if ( ! function_exists( 'wpcstore_is_woocommerce_activated' ) ) {
	/**
	 * Query WooCommerce activation
	 */
	function wpcstore_is_woocommerce_activated() {
 ...
 *
 */
function wpcstore_do_shortcode( $tag, array $atts = array(), $content = null ) {
 ...
 * Accounts for the WPCstore Designer and WPCstore Powerpack content background option.
 *
 * @return string the background color
 * @since  1.6.0
 */
function wpcstore_get_content_background_color() {
	if ( class_exists( 'WPCstore_Designer' ) ) {
		$content_bg_color = get_theme_mod( 'sd_content_background_color' );
		$content_frame    = get_theme_mod( 'sd_fixed_width' );
	}

	if ( class_exists( 'WPCstore_Powerpack' ) ) {
	...
	if ( class_exists( 'WPCstore_Powerpack' ) || class_exists( 'WPCstore_Designer' ) ) {
...
/**
 * Apply inline style to the WPCstore header.
 *
 * @uses  get_header_image()
 * @since  2.0.0
 */
function wpcstore_header_styles() {
	...
	$styles = apply_filters( 'wpcstore_header_styles', $styles );
...
/**
 * Apply inline style to the WPCstore homepage content.
 *
 * @uses  get_the_post_thumbnail_url()
 * @since  2.2.0
 */
function wpcstore_homepage_content_styles() {
	...
	$styles = apply_filters( 'wpcstore_homepage_content_styles', $styles );
 ...
 * @since  1.0.0
 */
function wpcstore_adjust_color_brightness( $hex, $steps, $opacity = 1 ) {
 ...
 * @since  1.3.0
 */
function wpcstore_sanitize_choices( $input, $setting ) {
...
add_filter( 'the_content_more_link', 'wpcstore_continue_reading_link' );
